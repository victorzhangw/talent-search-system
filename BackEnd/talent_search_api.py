...
class LLMService:
    ...
    def get_trait_analysis_prompt(self) -> str:
        ...
        return f"""你是一個專業的人才搜索助手。你的回應必須是有效的 JSON 格式。
...
**關鍵規則（違反將導致錯誤）**：
1. 第一個字符必須是 {{，最後一個字符必須是 }}
2. 不要在 JSON 前後添加任何文字、說明或標記
3. 不要使用 ```json 或 ``` 標記
4. 所有屬性名和字串值必須使用雙引號 "
5. 不要使用單引號 '
6. min_score 是數字類型，不要加引號
7. 不要在 JSON 中添加註釋
8. 確保所有括號正確配對
...
記住：你的整個回應必須是一個有效的 JSON 對象，可以直接被 Python 的 json.loads() 解析。"""
