...
#
# 2026-09-18（U5-5）：補上「交叉驗證」「多方驗證」，配合 V7 匯入。
# 2026-09-19：**「多方驗證」已移除**。它同時是規則八提醒句的用詞（「…之單一參考，
# 需多方驗證後決策」），而那句是制式樣板、與校準無關。只要模型寫了那句提醒，本檢查
# 就會判 passed——即使整篇沒有任何一句校準警語。實測：957 KB 回答語料裡「多方驗證」
# 出現 13 次，**13 次全部在提醒句內，0 次獨立使用**，所以它對本檢查的唯一貢獻就是
# 假通過。而且踩到的正是最該有校準警語的情境：高風險適任判斷 × 自述可信度低的人。
# V7 正本的措辭是「資訊可信度需交叉驗證」，由「交叉驗證」涵蓋，不需要「多方驗證」。
#
# 這四個詞原本與資料是一組的：V6.2 的三個校準特質 band A，K 欄尾句寫著
# 「其自述內容的可信度需**以行為事例佐證**」——資料示範用語，這裡驗模型有沒有照做。
# V7 把那句改成「資訊可信度需**交叉驗證**」，四個舊詞一個都不剩，於是變成
# 「資料叫模型寫 A、檢查卻要求模型寫 B」。補詞是讓兩邊重新對齊。
#
# 明確不宣稱的事：這**不是**在修 calibration_evidence 目前的失敗率（50 筆適用、
# 17 過 33 敗）。那個數字是 V6.2 資料還在線上時量到的——2026-08-18 失敗 13 次、
# 0831 失敗 9 次，而當時 payload 裡分別出現 16 次與 25 次「以行為事例佐證」。
# 句子在的時候就已經失敗了，成因另有其他，尚未定位。
