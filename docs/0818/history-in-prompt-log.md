...
3. [完成] `demo_prompt_log.py` 通過（V-11），其中 `HISTORY_TURNS` 斷言已更新為 `HISTORY_MSGS`，
   並補了兩條檢查表頭換算自洽的斷言。
4. [未完成] 以真實後端跑一段 **至少 7 輪** 的對話（超過 6 輪上限），取 `prompts.log` 檢查：
   第 7 輪的歷史區塊為 12 則、第 1 輪的問答已被丟棄、`HISTORY_MSGS: 12 (6 turns, cap=6 turns/12 msgs)`。
   8/18 的實際流量最多只跑到 4 輪，尚未觸及上限，待補一段更長的對話。
5. [未完成] 同一段對話取 `PROMPT_LOG_PER_SESSION=1` 產生的 per-session 檔，
   確認 7 輪依序落在同一個檔內、無其他 session 交錯。
   8/18 驗證時該旗標尚未開啟，`.env` 已於同日補上，需重啟後端後重跑。
...
7. [部分完成] 拿改造後任一筆記錄的 `[SYSTEM PROMPT]` 以下部分，與 v7 範例逐行 diff，
   ...
   8/18 實測四筆記錄的 `====` 之後結構完整，但與客戶三份 v7 範例的逐行 diff 尚未執行。）
...
資料來源：`BackEnd/api_v2/logs/2026-08-18/`，4 筆真實請求，
同一 session `87a6fe0e-443c-4f21-9659-fcc34123e02d` 連續四輪，歷史則數 0 → 2 → 4 → 6。

| 驗證項 | 結果 |
|---|---|
| 4 筆記錄都有 `[CONVERSATION HISTORY]` 區塊，位於表頭之後、`====` 之前 | 4/4 通過 |
| `HISTORY_MSGS` 數字與區塊實際列出的 `--- #n ---` 則數相符 | 4/4 通過 |
| role 序列為 `USER`／`ASSISTANT` 嚴格交替（提問與回覆都帶） | 4/4 通過 |
| 歷史內容與 `conversations.log` 的實際回覆**逐字相同**（3,167／1,524／1,467 字元） | 3/3 通過 |
| `REQ` 值在 `prompts.log`、`conversations.log` 的 `[USER]` 與 `[AI]`、`log_packer_audit.log` 四處齊備 | 4/4 通過 |
| `====` 之後緊接 `[SYSTEM PROMPT]`，且 `【輸入數據】`、`[任務指令]` 俱在 | 4/4 通過 |

**客戶原提問對應到的結論**：歷史確實帶入且可在 log 上直接讀到；提問與回覆兩邊都帶；
內容為原文逐字，無摘要或截斷。三點皆有真實資料佐證。

**尚未取得證據**：超過 6 輪上限後的截斷行為（DoD 第 4 條）。
截斷邏輯本身已由離線的 V-5 驗證（`cap=2` 時保留 `Q3 A3 Q4 A4`），
但真實流量還沒有一段對話長到觸發它。
