...
class PDFReportGenerator:
    ...
    def _draw_header_footer(self, canvas, doc):
        ...
        # 簡單的最後一頁判斷 - 回到固定第7頁
        is_last_page = page_num == 7
        
        print(f"[DEBUG] 當前頁: {page_num}, 是第7頁(最後一頁): {is_last_page}")
    ...
    def generate_test_result_report(self, test_result, output_path=None):
        ...
        def header_footer_with_count(canvas, doc):
            self.total_pages_count = max(self.total_pages_count, canvas.getPageNumber())
            self._draw_header_footer(canvas, doc)
        
        # 建立 PDF
        doc.build(story, onFirstPage=self._draw_cover_page_header_footer, onLaterPages=header_footer_with_count)
    ...
    def _create_cover_page(self, test_result):
        ...
        if test_result.test_invitation.completed_at:
            completed_time_tw = test_result.test_invitation.completed_at.astimezone(taiwan_tz)
            completed_time = completed_time_tw.strftime('%Y年%m月%d日 %H:%M')
        else:
            completed_time = "N/A"
...
職位相關條件、資源進行後續行動依據。"""
    ...
    def _create_key_analysis_section(self, test_result):
        ...
        for category_name, score in sorted_categories:
            ...
            # 建立主要項目內容
            main_text = f"<b>{category_name}{english_name}：{round(score)} 分</b>"
        ...
        story.append(Paragraph(f"<b>優勢面向：{max_category_name} ({round(max_score)} 分)</b>", advantage_subtitle_style))
        ...
        story.append(Paragraph(f"<b>劣勢面向：{min_category_name} ({round(min_score)} 分)</b>", advantage_subtitle_style))
    ...
    def _create_merged_trait_scores_section(self, test_result):
        ...
        # 收集所有特質資料，排除重複的系統對應名稱
        all_traits_data = []
        seen_system_names = set()  # 用於記錄已處理的系統名稱
        ...
        for category in categories:
            ...
            for trait in traits:
                # 跳過重複的系統對應名稱
                if trait.system_name in seen_system_names:
                    continue
                ...
                if isinstance(trait_score, (int, float)) and trait_score > 0:
                    # 記錄已處理的系統名稱
                    seen_system_names.add(trait.system_name)
        ...
        # 按中文名稱排序（字母順序）
        all_traits_data.sort(key=lambda x: x['name'])
    ...
    def _generate_radar_chart(self, category_scores):
        ...
        try:
            ...
            # 創建顯示標籤，包含英文名稱（與網頁版一致）
            display_categories = []
            for cat in categories:
                # 根據網頁版的雷達圖，標籤應該直接使用分類名稱
            ...
            ax.set_xticklabels(display_categories, fontsize=16, fontweight='bold')
