...
使用者已先登入Traitty系統（Django）。插件與 Traitty 會使用：
...
透過甲方 API 用 (account_id, Bearer Token) 解析出「公司編碼」（下稱 enterprise_code）。

使用 enterprise_code 取得該公司底下所有候選人。

使用者在 Traitty 前端可複選候選人，Traitty 後端將批次取得各候選人的：
...
前提：使用者已在 Traitty系統（Django）完成登入，登入態由 Django session cookie維持。

插件載入後，由後端服務呼叫甲方提供的端點取得「插件專用短效 token」（短效、限定 plugin scope，供 Traitty 驗證），返回前端：

POST /plugin-auth/token

認證方式：使用者已登入（瀏覽器自動帶 session cookie；插件不需也不應讀取 HttpOnly cookie）
...
Step 1：解析公司代碼 enterprise_code

Traitty 後端呼叫：POST /v1/enterprise/resolve

驗證 plugin_access_token 與 account/enterprise 關係

回傳：enterprise_code 與 行業/職位說明

Step 2：取得候選人清單

Traitty 後端呼叫：GET /v1/candidates?enterprise_code=...

甲方回傳候選人清單（每筆包含 candidate_id）

Step 3：複選候選人後，批次取得測驗報告 + 報告定義

Traitty 前端複選多位候選人

Traitty 後端呼叫：POST /v1/assessments/latest:batch

甲方回傳每位候選人的最新報告（含 trait_results.<trait_id>.semantic_band 與 trait_metadata.<trait_id>.semantic 的語意解說）

2. 通用規範

2.1 Base URL 與版本

Base URL（示例）：https://uat.traitty.com.tw
...
dimension_group	definition	hidden_anchor	band	band_range	semantic_label	semantic_description	management_focus	usage_note	report_wording_zh	report_wording_friendly	ai_do	ai_dontmanagement_focus, usage_note
...
用途：使用插件 token 中的 account_id、enterprise_id 解析出enterprise_code。
...
"enterprise_desc": "公司/行業性質說明",

"job_desc": [{"職位1":"說明"},{"職位2":"說明"}]
}
